		...
		daos_handle_t ioh, int status, struct dtx_handle *dth)
{
	...
	if (daos_handle_is_valid(ioh)) {
		...
		if (rc != 0) {
			D_CDEBUG(rc == -DER_REC2BIG || rc == -DER_INPROGRESS || rc == -DER_TX_RESTART || rc == -DER_EXIST ||
				     rc == -DER_NONEXIST || rc == -DER_ALREADY, DLOG_DBG, DLOG_ERR,
...
obj_set_reply_nrs(crt_rpc_t *rpc, daos_handle_t ioh, d_sg_list_t *echo_sgl)
{
	...
	D_ALLOC(orwo->orw_nrs.ca_arrays, nrs_count * (sizeof(uint32_t) + sizeof(daos_size_t)));
	if (orwo->orw_nrs.ca_arrays == NULL)
		return -DER_NOMEM;

	orwo->orw_nrs.ca_count = nrs_count;
	orwo->orw_data_sizes.ca_count = nrs_count;
	orwo->orw_data_sizes.ca_arrays = (void *)((char *)orwo->orw_nrs.ca_arrays + nrs_count * (sizeof(uint32_t)));
...
obj_echo_rw(crt_rpc_t *rpc, daos_iod_t *split_iods, uint64_t *split_offs)
{
	...
	if (obj_rpc_is_fetch(rpc)) {
		rc = obj_set_reply_sizes(rpc, orw->orw_iod_array.oia_iods, orw->orw_iod_array.oia_iod_nr);
		if (rc)
			D_GOTO(out, rc);
	}
		 ...
		 struct obj_iod_array *oia, daos_unit_oid_t *uoid)
{
	...
	for (i = 0; i < oia->oia_iod_nr; i++) {
		...
		if (rc != 0) {
			D_ERROR(DF_C_UOID_DKEY"iod[%d]: "DF_C_IOD" verify_key failed for akey: "DF_KEY", csum: "DF_CI", error: "DF_RC"\n",
				DP_C_UOID_DKEY(*uoid, dkey), i, DP_C_IOD(iod), DP_KEY(&iod->iod_name), DP_CI(csum->ic_akey), DP_RC(rc));
			return rc;
		}
	}
		      ...
		      uint64_t *split_offs, struct dtx_handle *dth)
{
	...
	iod_csums = split_csums == NULL ? orw->orw_iod_array.oia_iod_csums : split_csums;
...
/* 读流程 */
		...
		D_ASSERTF(ec_recov == false || ec_deg_fetch == false, "ec_recov %d, ec_deg_fetch %d.\n", ec_recov, ec_deg_fetch);

		/* BEG：EC恢复读 */ 
		if (ec_recov) {
			D_ASSERT(obj_ec_tgt_nr(&ioc->ioc_oca) > 0);
			is_parity_shard = is_ec_parity_shard_by_tgt_off(tgt_off, &ioc->ioc_oca);
			get_parity_list = ec_recov && is_parity_shard && ((orw->orw_flags & ORF_EC_RECOV_SNAP) == 0);
		}
		...
		if (unlikely(ec_recov && obj_ec_recov_need_try_again(orw, orwo, ioc))) {
			rc = -DER_FETCH_AGAIN;
			D_DEBUG(DB_IO, DF_UOID" "DF_X64"<"DF_X64" ec_recov needs redo, "DF_RC".\n",
				DP_UOID(orw->orw_oid), orw->orw_epoch, ioc->ioc_coc->sc_ec_agg_eph_boundry, DP_RC(rc));
			goto out;
		}
		/* END：EC恢复读 */
		...
		if (ec_deg_fetch && !spec_fetch) {
			...
			if (orw->orw_flags & ORF_EC_RECOV_FROM_PARITY) {
				if (shadows == NULL) {
					rc = -DER_IO;
					D_ERROR(DF_UOID" ORF_EC_RECOV_FROM_PARITY should not with NULL shadows, "DF_RC"\n", DP_UOID(orw->orw_oid), DP_RC(rc));
					goto out;
				}
		...
		//  核心函数
		rc = vos_fetch_begin(ioc->ioc_vos_coh,   // coh
		                     orw->orw_oid,       // object id
				             orw->orw_epoch, 
				             dkey, 
				             orw->orw_nr,        // iod_nr: akey的数量
				             iods,               // orw->orw_iod_array.oia_iods
				             cond_flags | fetch_flags, 
				             shadows,
				             &ioh, 
				             dth);
		...
		if (rc) {
			D_CDEBUG(rc == -DER_INPROGRESS || rc == -DER_NONEXIST || rc == -DER_TX_RESTART, DB_IO, DLOG_ERR, "Fetch begin for "DF_UOID" failed: "DF_RC"\n", DP_UOID(orw->orw_oid), DP_RC(rc));
			goto out;
		}
		...
		if (get_parity_list) {
			...
			if (parity_list != NULL) {
				daos_recx_ep_list_set(parity_list, orw->orw_nr, ioc->ioc_coc->sc_ec_agg_eph_boundry, 0);
		...
		if (rma) {
            ...
            // 赋值orwo->orw_nrs.ca_count、orwo->orw_data_sizes.ca_count、orwo->orw_data_sizes.ca_arrays 
			rc = obj_set_reply_nrs(rpc, ioh, NULL);
			if (rc != 0)
				goto out;
		} else {
		    // orw->orw_sgls.ca_arrays
		...
		if (recov_lists != NULL) {
			 ...
			 * Or, will recovery from max{parity_epoch, vos_epoch_boundary}.
			 */
			...
			daos_recx_ep_list_set(recov_lists, orw->orw_nr, recov_epoch, recov_snap);
...
/* 读流程 */
    ...
    //  读流程
	if (obj_rpc_is_fetch(rpc) && !spec_fetch && daos_csummer_initialized(ioc->ioc_coc->sc_csummer)) {

	    // 初始化校验和放入orwo->orw_iod_csums.ca_arrays中
    ...
    /* 数据有2种类型的： crt_bulk_t(orw_bulks)和d_sg_list_t(orw_sgls) */ 
	if (rma) {
		// bulk
		bulk_bind = orw->orw_flags & ORF_BULK_BIND;
		...
		if (rc == 0) {
			 ...
			 */
			if (obj_rpc_is_update(rpc) && !(orw->orw_flags & ORF_RESEND) && DAOS_FAIL_CHECK(DAOS_DTX_RESEND_DELAY1))
				rc = dss_sleep(3100);
		}
	} 
	...
	if (rc) {
		if (rc == -DER_OVERFLOW)
			rc = -DER_REC2BIG;
		D_CDEBUG(rc == -DER_REC2BIG, DLOG_DBG, DLOG_ERR, DF_UOID" data transfer failed, dma %d rc "DF_RC"", DP_UOID(orw->orw_oid), rma, DP_RC(rc));
		D_GOTO(post, rc);
	}
...
// 写操作
	if (obj_rpc_is_update(rpc)) {
		...
		// CSUM Verified on update, now corrupt to fake corruption on disk
...
out:
	 ...
	 */
	if (rc == 0 && obj_rpc_is_update(rpc) && dth->dth_need_validation && sched_cur_seq() != sched_seq) {
		daos_epoch_t	epoch = 0;
		int		rc1;

		rc1 = dtx_handle_resend(ioc->ioc_vos_coh, &orw->orw_dti, &epoch, NULL);
		switch (rc1) {
			case 0:            // 当前DTX已经prepared
				orw->orw_epoch = epoch;
				/* Fall through */
			case -DER_ALREADY: // 当前DTX已经committed或者committable
				rc = -DER_ALREADY;
				break;
			case -DER_NONEXIST:
			case -DER_EP_OLD:
...
again:
	rc = obj_local_rw_internal(rpc, ioc, split_iods, split_csums, split_offs, dth);
	...
	if (dth != NULL && obj_dtx_need_refresh(dth, rc)) {
		...
		if (unlikely(++count % 10 == 3)) {
			...
			dsp = d_list_entry(dth->dth_share_tbd_list.next, struct dtx_share_peer, dsp_link);
...
ds_obj_tgt_update_handler(crt_rpc_t *rpc)
{
	...
	if (DAOS_FAIL_CHECK(DAOS_VC_LOST_DATA)) {
		if (orw->orw_dti_cos.ca_count > 0) {
			rc = vos_dtx_commit(ioc.ioc_vos_coh, orw->orw_dti_cos.ca_arrays, orw->orw_dti_cos.ca_count, NULL);
			if (rc < 0) {
				D_WARN(DF_UOID": Failed to DTX CoS commit "DF_RC".\n", DP_UOID(orw->orw_oid), DP_RC(rc));
			} else if (rc < orw->orw_dti_cos.ca_count) {
				D_WARN(DF_UOID": Incomplete DTX CoS commit rc = %d expected %" PRIu64 ".\n", DP_UOID(orw->orw_oid), rc, orw->orw_dti_cos.ca_count);
			}
		}
		D_GOTO(out, rc = 0);
	}
	...
	rc = dtx_begin(ioc.ioc_vos_coh, &orw->orw_dti, &epoch, 1,
		           orw->orw_map_ver, &orw->orw_oid, orw->orw_dti_cos.ca_arrays,
		  ...
		  dtx_sub_comp_cb_t comp_cb)
{
	...
	if (idx == -1) {
		...
		rc = obj_local_rw(exec_arg->rpc, exec_arg->ioc, iods, csums, offs, &dlh->dlh_handle);
		if (rc != 0)
			D_CDEBUG(rc == -DER_INPROGRESS || rc == -DER_TX_RESTART ||
				    (rc == -DER_EXIST && (orw->orw_api_flags & (DAOS_COND_DKEY_INSERT | DAOS_COND_AKEY_INSERT))) ||
				    (rc == -DER_NONEXIST && (orw->orw_api_flags & (DAOS_COND_DKEY_UPDATE | DAOS_COND_AKEY_UPDATE))),
...
ds_obj_rw_handler(crt_rpc_t *rpc)
{
	...
	if (tgt_cnt == 0) {
		...
		dtx_flags |= DTX_SOLO; //单副本模式，实际cnt=1,就是自己
	}

	if (orw->orw_flags & ORF_DTX_SYNC)
		dtx_flags |= DTX_SYNC; 
	    // 设置dth->dth_sync, 
	    // 设置后会Commit the given DTX array globally
...
again2:
    ...
    // orw->orw_iod_array.oia_oiods != NULL 为EC， EC处理
	if (orw->orw_iod_array.oia_oiods != NULL && split_req == NULL) {  // split_req用来判断重试的不在进行拆分
	...
	dti_cos_cnt = dtx_list_cos(ioc.ioc_coc, &orw->orw_oid, orw->orw_dkey_hash, DTX_THRESHOLD_COUNT, &dti_cos);
	if (dti_cos_cnt < 0)
	...
	if (opc == DAOS_OBJ_RPC_UPDATE && !(orw->orw_flags & ORF_RESEND) && DAOS_FAIL_CHECK(DAOS_DTX_LOST_RPC_REPLY))
		ioc.ioc_lost_reply = 1;
out:
	if (unlikely(rc != 0 && need_abort)) { // 读流程即使读失败也不会abort, 读流程不会执行
		  ...
		  struct obj_io_context *ioc, struct dtx_handle *dth)
{
	 ...
	 * Let's check resent again before further process.
	 */
	if (rc == 0 && dth->dth_modification_cnt > 0 && dth->dth_need_validation && sched_cur_seq() != sched_seq) {
		daos_epoch_t	epoch = 0;
		int		rc1;
